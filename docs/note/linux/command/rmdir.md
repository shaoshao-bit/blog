...
目录列表：要删除的空目录列表。当要删除多个空目录时，目录名之间使用空格隔开。

### 实例

将工作目录中，名为`www`的子目录删除
...
在工作目录下的www目录中，删除名为Test的子目录。若Test删除后，www目录成为空目录，则www亦予删除。

```bash
rmdir -p www/Test
```

下面的命令等价于`rmdir a/b/c`,`rmdir a/b`,`rmdir a`
